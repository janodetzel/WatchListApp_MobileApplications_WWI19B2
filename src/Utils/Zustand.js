  ...
  persist(
    (set, get) => ({
      ...
      getUserByName: (userName) =>
        Object.keys(get().users).find(
          (key) =>
            get().users[key].name.toUpperCase() === userName.toUpperCase()
        ),
      ...
      addCard: (cardDetails, cardListKey) =>
        set((state) =>
          produce(state, (draft) => {
            draft.users[draft.currUserKey].cardLists[cardListKey].cards[
              uuidv4()
            ] = {
              details: cardDetails,
              created: new Date(),
      ...
      checkCard: (check, cardKey, cardListKey) =>
        set((state) =>
          produce(state, (draft) => {
            draft.users[draft.currUserKey].cardLists[cardListKey].cards[
              cardKey
      ...
      deleteCard: (cardKey, cardListKey) =>
        set((state) =>
          produce(state, (draft) => {
            delete draft.users[draft.currUserKey].cardLists[cardListKey].cards[
              cardKey
            ];
          })
        ),
